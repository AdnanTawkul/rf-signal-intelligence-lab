...
from rfsil.ssl.contrastive import ProjectionHeadConfig
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Pretrain an RF IQ encoder using VICReg "
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/pretrain_vicreg_v1.yaml"),
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=None,
        help="Optional epoch override for smoke tests.",
    )
    parser.add_argument(
        "--output-directory",
        type=Path,
...
def load_yaml(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(content, dict):
        raise ValueError(
            "VICReg configuration must be a YAML mapping."
        )
...
def create_encoder_configuration(
    ...
    """Create the CNN encoder configuration."""
    return BaselineCNNConfig(
        in_channels=int(content.get("in_channels", 2)),
        num_classes=int(content.get("num_classes", 4)),
        channels=tuple(
            int(value)
        ...
        kernel_size=int(content.get("kernel_size", 7)),
        dropout=float(content.get("dropout", 0.2)),
        normalize_input_rms=bool(
            content.get("normalize_input_rms", False)
        ),
        normalization=str(
            content.get("normalization", "group")
        ),
        group_norm_groups=int(
            content.get("group_norm_groups", 8)
        ),
    )
...
def create_projection_configuration(
    ...
    return ProjectionHeadConfig(
        hidden_dimension=int(
            content.get("hidden_dimension", 256)
        ),
        output_dimension=int(
            content.get("output_dimension", 128)
        ),
    )
...
def create_augmentation_configuration(
    ...
    return IQAugmentationConfig(
        ...
        amplitude_scale_min=float(
            content.get("amplitude_scale_min", 0.8)
        ),
        amplitude_scale_max=float(
            content.get("amplitude_scale_max", 1.25)
        ),
        ...
        awgn_probability=float(
            content.get("awgn_probability", 0.5)
        ),
        awgn_snr_db_min=float(
            content.get("awgn_snr_db_min", 18.0)
        ),
        awgn_snr_db_max=float(
            content.get("awgn_snr_db_max", 30.0)
        ),
    )
...
def create_loss_configuration(
    ...
    """Create the VICReg objective configuration."""
    return VICRegLossConfig(
        invariance_weight=float(
            content.get("invariance_weight", 25.0)
        ),
        variance_weight=float(
            content.get("variance_weight", 25.0)
        ),
        covariance_weight=float(
            content.get("covariance_weight", 1.0)
        ),
...
def validate_contrastive_batching(
    ...
    """Prevent invalid VICReg batches."""
    if batch_size < 2:
        raise ValueError(
            "VICReg batch size must be at least two."
        )
...
def metrics_to_record(
    epoch: int,
    training: VICRegEpochMetrics,
    validation: VICRegEpochMetrics,
) -> dict[str, float | int]:
    """Serialize one epoch of VICReg metrics."""
    return {
        "epoch": epoch,
        "training_total_loss": training.total_loss,
        "training_invariance_loss": training.invariance_loss,
        "training_variance_loss": training.variance_loss,
        "training_covariance_loss": training.covariance_loss,
        ...
        "validation_invariance_loss": validation.invariance_loss,
        "validation_variance_loss": validation.variance_loss,
        "validation_covariance_loss": validation.covariance_loss,
...
def plot_history(
    ...
    """Plot VICReg losses and representation diagnostics."""
    epochs = [
        int(record["epoch"])
    ...
    axes[0].plot(
        epochs,
        [
            float(record["training_total_loss"])
            for record in history
        ],
        label="Training",
    )
    ...
    axes[0].set_title("VICReg pretraining")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()

    component_specs = (
        (
            "training_invariance_loss",
            "Train invariance",
        ),
        (
            "validation_invariance_loss",
            "Validation invariance",
        ),
        (
            "training_variance_loss",
            "Train variance",
        ),
        (
            "validation_variance_loss",
            "Validation variance",
        ),
        (
            "training_covariance_loss",
            "Train covariance",
        ),
        (
            "validation_covariance_loss",
    ...
    axes[1].set_ylabel("Unweighted component loss")
    ...
    axes[2].set_ylabel("Positive cosine similarity")
    ...
    axes[3].set_ylabel("Mean projection std.")
...
def main() -> None:
    ...
    seed = int(content.get("seed", 2026))
    ...
    for parameter in encoder.classifier.parameters():
    ...
    print("Labels used during pretraining: no")
    print(
        "Loss weights: "
        f"invariance={loss_configuration.invariance_weight:g}, "
        f"variance={loss_configuration.variance_weight:g}, "
        f"covariance={loss_configuration.covariance_weight:g}"
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (
            ...
            best_model_state = state_dict_to_cpu(
                model
            )
            best_encoder_state = state_dict_to_cpu(
                model.encoder
            )
            best_projection_state = state_dict_to_cpu(
        ...
        print(
            ...
            f"val inv "
            f"{validation_metrics.invariance_loss:.4f} | "
            f"val var "
            f"{validation_metrics.variance_loss:.4f} | "
            f"val cov "
            f"{validation_metrics.covariance_loss:.4f} | "
            f"val positive "
    ...
    torch.save(
        {
            ...
            "best_validation_invariance_loss": (
                best_validation_metrics.invariance_loss
            ),
            "best_validation_variance_loss": (
                best_validation_metrics.variance_loss
            ),
            "best_validation_covariance_loss": (
                best_validation_metrics.covariance_loss
            ),
    ...
    summary = {
        ...
        "final_training_total_loss": float(
            final_record["training_total_loss"]
        ),
        "final_validation_total_loss": float(
            final_record["validation_total_loss"]
        ),
    ...
    print(
        ...
        f"{best_validation_total_loss:.4f}"
    )
    print(
        "Best validation invariance loss: "
        f"{best_validation_metrics.invariance_loss:.4f}"
    )
    print(
        "Best validation variance loss: "
        f"{best_validation_metrics.variance_loss:.4f}"
    )
    print(
        "Best validation covariance loss: "
        f"{best_validation_metrics.covariance_loss:.4f}"
    )
    print(
