...
from rfsil.dsp.impairments import apply_phase_offset
...
def main() -> None:
    phase_offset_degrees = 35.0
    phase_offset_rad = float(np.deg2rad(phase_offset_degrees))
    ...
    shifted_samples = apply_phase_offset(
        signal.samples,
    ...
    output_path = (
        ...
        / "reports"
        / "figures"
        / "phase_offset_constellation.png"
    )
    ...
    axes[1].set_title(f"QPSK with {phase_offset_degrees:.1f}? Phase Offset")
    ...
    figure.suptitle("Constant Carrier Phase Offset")
    figure.tight_layout()
    ...
    print(f"Phase offset degrees: {phase_offset_degrees:.1f}")
    print(f"Phase offset radians: {phase_offset_rad:.6f}")
